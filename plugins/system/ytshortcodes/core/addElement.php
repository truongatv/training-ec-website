...
Class AddElementShortcodes{
	...
	public static function yt_shortcodes_FormElement($element,$name,$desc,$value = ''){
		...
		$buildShortCode = 'var tagOpen = $(".yt_shortcodes_open").text();
					   ...
					   var group_source = $(".yt-generator-isp-sources").val();
					   ';
		$jinput = JFactory::getApplication()->input;
		if($jinput->get('option') == "com_content")	
		{
			$buildShortCode .=	'var id_article = $("#jform_id").val();';
		}	
		if($jinput->get('option') == "com_k2")	
		{
			$buildShortCode .=	'var id_article = $("input[name=id]").val();';
		}			
		if($jinput->get('option') == "com_virtuemart")
		{
			$buildShortCode .=	'var id_article = $("input[name=virtuemart_product_id]").val();';
		}
		if($element == "content_style")
		{
			$buildShortCode .='html +=" article_id=\""+id_article+"\"";';
		}			
		$buildShortCode .='$(".yt_shortcodes_parent_form_element .yt-generator-field-container").each(function (index)
						{
		...
		if(!is_dir(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'shortcodes'.DIRECTORY_SEPARATOR.$element.'_item'))
		{
			...
			if($check_content ==1)
			{
				...
				$buildShortCode .= 'html += " "; return html;';
			}
		}
			...
			foreach($field_form as $index => $field){
				...
				if(isset($field['child']) && count($field['child']) > 0)
				{
					...
					$html .='<div class="yt-generator-field-group" '.$data_group.'>';
						$html .='<div class="yt-generator-field-container '.$class_field.' yt-field-type-'.$field['type'].' yt-generator-skip" data-default="'.$field['default'].'" data-name="'.$index.'" '.$data_group.'>
					<h5>'.$field['name'].'</h5>';
						...
						foreach($field['child'] as $index__ => $field__)
						{
							...
							$html .='<div class="yt-generator-field-container '.$class_field.' yt-field-type-'.$field__['type'].' yt-generator-skip" data-default="'.$field__['default'].'" data-name="'.$index__.'" '.$data_group_son.'>
							<h5>'.$field__['name'].'</h5>';
				...
				{
					$html .='<div class="yt-generator-field-container field-group-1 yt-field-type-'.$field['type'].' yt-generator-skip" data-default="'.$field['default'].'" data-name="'.$index.'" '.$data_group.'>
					<h5>'.$field['name'].'</h5>';
					$html .= YT_Field_Shortcodes::formField($index,$field);
					if($index != "content")
					{
		...
		if(file_exists(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'shortcodes'.DIRECTORY_SEPARATOR.$element.'_item'.DIRECTORY_SEPARATOR.'config.php'))
		{
			...
			$checkcontentson = 0;
			 ...
			 var group1 = $("#yt-generator-attr-type_change").val();
			 var group_source1 = $(".yt-generator-isp-sources").val();
			 for(i = 0; i < length; i++)
			 {
				...
				for (var a = 0;a < $(".yt_shortcodes_son_form_element .yt_shortcodes_son_wrap").eq(i).find(".yt_shortcodes_wrap_form .yt-generator-field-container_son .yt-generator-field-container").length; a++){
					data_group1 = $(".yt_shortcodes_son_form_element .yt_shortcodes_son_wrap").eq(i).find(".yt_shortcodes_wrap_form .yt-generator-field-container_son .yt-generator-field-container").eq(a).attr("data-group");
					if(data_group1 == group1 || data_group1=="all" || data_group1 == group_source1)
					{
			...
			foreach($field_form_son as $index => $field)
			{
				...
				$html .= '<div class="yt-generator-field-container_son">';
					if(isset($field['child']) && count($field['child']) > 0){
						...
						$html .='<div class="yt-generator-field-group" '.$data_group.'>';
							$html .='<div class="yt-generator-field-container '.$class_field1.' yt-field-type-'.$field['type'].' yt-generator-skip" data-default="'.$field['default'].'" data-name="'.$index.'" '.$data_group.'>
						<h5>'.$field['name'].'</h5>';
							$html .= YT_Field_Shortcodes::formField($index,$field);
							$html .='<div class="yt-generator-attr-desc">'.$field['desc'].'</div></div>';
							foreach($field['child'] as $index__ => $field__){
								...
								$html .='<div class="yt-generator-field-container '.$class_field1.' yt-field-type-'.$field__['type'].' yt-generator-skip" data-default="'.$field__['default'].'" data-name="'.$index__.'" '.$data_group.'>
								<h5>'.$field__['name'].'</h5>';
								$html .= YT_Field_Shortcodes::formField($index__,$field__);
								if($index != "content")
								{
					...
					}else
					{
						...
						$html .='<div class="yt-generator-field-container field-group-1 yt-field-type-'.$field['type'].' yt-generator-skip" data-default="'.htmlentities($field['default']).'" data-name="'.$index.'" '.$data_group.'>
						<h5>'.$field['name'].'</h5>';
						$html .= YT_Field_Shortcodes::formField($index,$field);
						if($index != "content")
						{

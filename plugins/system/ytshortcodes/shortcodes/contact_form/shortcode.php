...
function contact_formYTShortcode ($atts,$content = null)
{
	...
	if($icon_name != ''){
		...
		$icon_name = "<span class='add-on'><i class='fa fa-" . trim(str_replace('icon:', '', $icon_name)) . "'></i></span> ";
		$css .=".".$iduniq." .input-name .add-on{background:".$background_name."; color:".$color_name."}";
		}else{
			$icon_name = '<span class="add-on"><img src="'.yt_image_media($icon_name).'" style="width:18px" alt="" /></span> ';
			$css .=".".$iduniq." .input-name .add-on{background:".$background_name."; color:".$color_name."}";
		}
	}
	if($icon_email != ''){
		if (strpos($icon_email, 'icon:') !== false) {
			$icon_email = "<span class='add-on'><i class='fa fa-" . trim(str_replace('icon:', '', $icon_email)) . "'></i></span> ";
			$css .=".".$iduniq." .input-email .add-on{background:".$background_email."; color:".$color_email."}";
		}else{
			$icon_email = '<span class="add-on"><img src="'.yt_image_media($icon_email).'" style="width:18px" alt="" /></span> ';
			$css .=".".$iduniq." .input-email .add-on{background:".$background_email."; color:".$color_email."}";
		}
	}
	if($icon_subject != '')	{
		if (strpos($icon_subject, 'icon:') !== false) {
			$icon_subject = "<span class='add-on'><i class='fa fa-" . trim(str_replace('icon:', '', $icon_subject)) . "'></i></span> ";
			$css .=".".$iduniq." .input-subject .add-on{background:".$background_subject."; color:".$color_subject."}";
		}else{
			$icon_subject = '<span class="add-on"><img src="'.yt_image_media($icon_subject).'" style="width:18px" alt="" /></span> ';
			$css .=".".$iduniq." .input-subject .add-on{background:".$background_subject."; color:".$color_subject."}";
		}
	}
	if($add_field != ''){
		...
		if(count($field_array) >0)
		{
			...
			for ($i=0; $i<count($field_array);$i++)
			{
				...
				if(count($str1_array) >4)
				{
					...
					{
						$icon_add = '<span class="add-on"><i class="fa fa-'.$str1_array[6].'"></i></span>';
						$css .=".".$iduniq." .input-".$str1_array[0]." .add-on{background:".$str1_array[4].";color:".$str1_array[3]."}";
					}
					$addstr.='
					<div class="form-group input-'.$str1_array[0].'">';
					if (strtolower($label_show)=='yes' || strtolower($label_show)=='no|yes'){
						$addstr.='<label for="'.$str1_array[0].'" class="yt-form-label">'.$str1_array[0].':</label>';
						$css .=".".$iduniq." .input-".$str1_array[0]." .yt-form-label{color:".$str1_array[3]."}";
					}
					$addstr.='
					<div class="yt-input-box '.($icon_add !="" ? "yt-input-prepend" : "").'">';
					if($str1_array[1] == 'textarea'){
						$addstr .='<textarea id="'.$str1_array[0].' "  placeholder="'.$str1_array[2].'"  class="form-control-1 '.$idman.'" name="'.$str1_array[0].'"></textarea>';
						$css .=".".$iduniq." .input-".$str1_array[0]." textarea{height: ".$textarea_height."px; margin-bottom:10px; background:".$str1_array[4].";color:".$str1_array[3]." !important}";
					}else{
						$addstr.= $icon_add;
						$addstr.='<input type="'.$str1_array[1].'" placeholder="'.$str1_array[2].'" id="'.$str1_array[0].'" class="form-control-1 '.$idman.'" name="'.$str1_array[0].'" style="" />';
						$css .=".".$iduniq." .input-".$str1_array[0]." input{background:".$str1_array[4].";margin-bottom:10px;color:".$str1_array[3]."}";
					}
				$addstr.='</div>
					</div>';
				}
        ...
        $str = '
        <div class="yt-clearfix yt-contact_form '.$type.' '.$fields .' '.$iduniq.'" style="'.$margin.'">
			...
			<form name="contact_us_form'.$iduniq.'" id="contact_us_form'.$iduniq.'" class="yt-clearfix contact_us_form " action="' . JURI::current() . '" method="POST">
				<div class="yt-form-fields">
					...
					$str.='
						...
						if (strtolower($label_show)=='yes' || strtolower($label_show)=='no|yes') {
						$str.='<label for="name'.$iduniq.'" class="yt-form-label">Your Name:</label>';
						$css .=".".$iduniq." .input-name .yt-form-label{color:".$color_name."}";
						}
						$str.='
						<div class="yt-input-box '.($icon_name !="" ? "yt-input-prepend" : "").'">
							'.$icon_name.'
							<input type="text" placeholder="Name" id="name'.$iduniq.'" class="form-control-1" name="name"/>
						</div>
						</div>';
						$css .=".".$iduniq." .input-name input{background:".$background_name.";color:".$color_name." !important ; }";
					}
					$str.='
					<div class="form-group input-email">
						';
						if (strtolower($label_show)=='yes') {
							$str.='<label for="email'.$iduniq.'" class="yt-form-label">Your E-Mail:</label>';
							$css .=".".$iduniq." .input-email .yt-form-label{color:".$color_email."}";
						}
						$str.='
						<div class="yt-input-box '.($icon_email !="" ? "yt-input-prepend" : "").'">
							 '.$icon_email.'
							<input type="text" placeholder="Email"  id="email'.$iduniq.'" class="form-control-1" name="email" style=""  />
						</div>
					</div>';
					$css .=".".$iduniq." .input-email input{background:".$background_email.";color:".$color_email."}";
					...
					$str.='
						<div class="form-group input-subject">
							';
							if (strtolower($label_show)=='yes') {
								$str.='<label for="subject'.$iduniq.'" class="yt-form-label">Your Subject:</label>';
								$css .=".".$iduniq." .input-subject .yt-form-label{color:".$color_subject."}";
							}
							$str.='
							<div class="yt-input-box '.($icon_subject !="" ? "yt-input-prepend" : "").'">
							  '.$icon_subject.'
								<input type="text" placeholder="Subject"  id="subject'.$iduniq.'" class="form-control-1" name="subject">
							</div>
						</div>';
						$css .=".".$iduniq." .input-subject input{background:".$background_subject.";color:".$color_subject."}";
				}
				$str.='
				</div>';
				$str.=$addstr;
				...
				<div class="yt-form-common-field">
					<div class="form-group input-message">';
						if (strtolower($label_show)=='yes' || strtolower($label_show)=='no|yes') {
							$str.='<label for="message'.$iduniq.'" class="yt-form-label">Your Message:</label>';
							$css .=".".$iduniq." .input-message .yt-form-label{color:".$color_message."}";
						}
						$str.='
						<div class="yt-input-box">
							<textarea id="message'.$iduniq.'"  placeholder="Message"  class="form-control-1 message" name="message"></textarea>
						</div>
					</div>
					...
					<div style="display: none;"></div>
						...
						<div class="form-group">
							<div class=" submit-button">
								';
								$css .=".".$iduniq." .input-message textarea{height: ".$textarea_height."px; background:".$background_message.";color:".$color_message."}";

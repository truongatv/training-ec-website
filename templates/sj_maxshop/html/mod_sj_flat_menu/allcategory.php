	...
	for($i = 0; $i < count($menus); $i++){
		...
		$li = "<li ".$class.">";
 			$li .= "<div class='fm-item '>";
				...
				$icon_menu = ($menus[$i]->menu_image != '')?'<img src='.JURI::base().$menus[$i]->menu_image.' alt="'.$menus[$i]->title.'"  />':'';
				$divLink = "<div class='fm-link' ".$style."><a class='magamenu-title' ".$target." href='".$menus[$i]->flink."'>".$menus[$i]->title."</a><a class='megamenu-icon' >".$icon_menu."</a></div>";
			$li.=$divLink;
			$li.= "</div>";
			echo $li;
				if($type_menu == 'flyout') {

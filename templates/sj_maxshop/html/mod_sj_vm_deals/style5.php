...
if (!empty($list)) {
	...
	echo '<div class="sj_vm_deals_wrap sj_relative style5" id="'.$module_id.'">';
		...
		echo '</div>';
		echo '<div class="sj-vm-deals-content">';
		
		if(($params->get('item_sub_category_display') == 1 && count($category_info) > 0) || ($params->get('item_image_category_display') == 1)) {
			...
			if($params->get('item_sub_category_display') == 1 && count($category_info) > 0){
						echo '<div class="item-inner-title-module-sub-category"><ul>';
							foreach($category_info as $index => $ci){
								if($index != 0){
									echo '<li class="item-inner-title-module-sub-category-item"><a href="'.$ci->link.'">'.$ci->title.'</a></li>';
								}
							}
